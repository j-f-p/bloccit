  ...
  describe "voting" do
    ...
    describe "#update_rank" do
      it "calculates the correct rank" do
        ...
        expect(post.rank).to eq (post.points +
          (post.created_at - Time.new(1970,1,1)) / 1.day.seconds)
      end
    ...
    describe "after_create" do
      ...
      it "sends an email to the user that created the post" do
        @another_post = topic.posts.new(title: RandomData.random_sentence,
          body: RandomData.random_paragraph, user: user)
        expect(FavoriteMailer).to receive(:new_post).with(user,
          @another_post).and_return(double(deliver_now: true))
        @another_post.save!
      end
    end
  end
